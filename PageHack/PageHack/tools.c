...
BOOLEAN QuerySymbolicLink(PUNICODE_STRING usName, PUNICODE_STRING usDestination)
{
	...
	// open symbolic link by name
	ns = ZwOpenSymbolicLinkObject(&h, GENERIC_READ, &ObjAttr);
	if (NT_SUCCESS(ns))
	{
		PWSTR Buff =NULL;
		USHORT wSize = 255 * sizeof(WCHAR);
query:
		// get target object by symbolic link
		Buff = (PWSTR)M_ALLOC(wSize);
		if (Buff)
		{
			ULONG RetLen = 0;
...
BOOLEAN RegSetValueKey(HANDLE hKey, PWSTR lpwcName, ULONG Type, PVOID Data, ULONG DataSize)
{
	...
	NTSTATUS ns;
	RtlInitUnicodeString(&usValueName, lpwcName);
	ns = ZwSetValueKey(hKey, &usValueName, NULL, Type, Data, DataSize);
	if (!NT_SUCCESS(ns))
	{
...
BOOLEAN AllocatePteTable(PVOID *Address, PULONG PfnValue)
{
    ...
    {
		PHYSICAL_ADDRESS MemAddr;
        memset(MdlInfo.MappedBuffer, 0, PAGE_SIZE);

        MemAddr = MmGetPhysicalAddress(MdlInfo.MappedBuffer);
...
BOOLEAN SetPfnForPagePae(PVOID Pointer, PHYSICAL_ADDRESS *PhysicalAddr)
{
    ...
    if (Pde->u.Hard.Valid)
    {
        ...
        if (Pde->u.Hard.LargePage != 0)
        {
            ...
            PteNumber = ((ULONG)Pointer & 0x001FF000) >> PAGE_SHIFT;
...
BOOLEAN SetPfnsForAddress(PVOID Address, ULONG PagesCount, PHYSICAL_ADDRESS *PhysicalAddr)
{
    ...
    PdeAddr = MmGetPhysicalAddress((PVOID)PDE_BASE_PAE);
